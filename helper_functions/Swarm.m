function [max_kernel_density_y] = Swarm(x, y, varargin);
...
% OUTPUT:
% max_kernel_density_y: max value of kernel density y (height) - used to plot significance bars above graph at proper height
...
switch lower(DistributionStyle) % Because switch has no case-invariant mode
    ...
    case 'violin'
        [violin_x, violin_y] = ksdensity(SwarmY, 'NumPoints', 100, 'Bandwidth', KernelDensityBW);
        ...
        max_kernel_density_y = max(violin_y);
...
function ParseVarargin()
    if ~isempty(varargin)
        ...
        for n = 1:nargin
            ...
            elseif strcmpi(varargin{1,n}, 'KernelDensityBW') || strcmpi(varargin{1,n}, 'KDBW')
                KernelDensityBW = varargin{2,n};
